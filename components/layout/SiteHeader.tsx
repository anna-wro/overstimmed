...
export function SiteHeader({ showBackButton, backButtonHref = "/" }: SiteHeaderProps) {
  ...
  const isLeavingTrackWithUnsaved =
    pathname === "/track" && trackUnsaved?.hasUnsaved

  const handleLogoClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    if (isLeavingTrackWithUnsaved) {
      e.preventDefault()
      trackUnsaved?.openDialog("/")
    }
  }
    ...
    <header className="container max-w-5xl mx-auto px-4 py-8">
        ...
        <Link
          ...
          aria-label={
            isLeavingTrackWithUnsaved
              ? "Dashboard (unsaved changes will prompt to save or discard)"
              : "Overstimmed home"
          }
        >
      ...
      <div className="flex justify-center items-center gap-4">
        ...
        <nav className="flex items-center gap-2 sm:gap-4 md:gap-6 px-4 py-2">
          ...
          <NavLink href="/track" icon={<Activity />} label="Track" />
          <NavLink href="/insights" icon={<LineChart />} label="Insights" />
          <NavLink href="/archive" icon={<CalendarDays />} label="Archive" />
